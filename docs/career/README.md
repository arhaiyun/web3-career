# 职业规划与求职材料

本目录用于沉淀 2026 年新阶段职业规划、简历、面试准备和 Web3 转型材料。

## 当前定位

**AI Agent / 风控平台架构专家**

- 12 年技术研发与平台架构经验。
- 北京理工大学 985 本硕。
- 小红书内容安全 AI 审核平台 POC / Owner。
- 阿里巴巴风控中台与营销反作弊经验。
- 主攻 AI Agent 工程化、内容安全 AI 平台、风控平台架构。
- 派生方向为 Web3 Risk AI、Crypto Fraud、Trust & Safety AI、Compliance Agent。

## 文档索引

| 文档 | 用途 |
|---|---|
| [大厂 AI 架构版中文简历](resume-ai-architect.md) | 主简历，投大厂 AI Agent、AI 应用架构、内容安全、风控平台岗位 |
| [Web3 Risk AI 派生版简历](resume-web3-risk-ai.md) | 投交易所、钱包、链上数据、合规科技、Web3 Trust & Safety 岗位 |
| [岗位定位与投递策略](job-positioning.md) | 判断投什么、不投什么、怎么介绍自己 |
| [面试故事库](interview-story-bank.md) | 10 个 STAR 故事，覆盖小红书、阿里、AI Agent、Web3 迁移 |
| [8 周求职与面试准备计划](8-week-interview-plan.md) | 7-8 月准备节奏，9 月开始投递 |
| [旧版简历 PDF](../周海云的简历_北理工_阿里.pdf) | 2023 年旧版简历，作为经历事实来源 |

## 推荐执行顺序

1. 先精修 `resume-ai-architect.md`，形成可投大厂主简历。
2. 再精修 `resume-web3-risk-ai.md`，形成 Web3 定制版。
3. 每天从 `interview-story-bank.md` 选一个故事，练 2 分钟口述。
4. 每周按 `8-week-interview-plan.md` 产出一个明确资产。
5. 投递前用 `job-positioning.md` 判断岗位匹配度和简历版本。

## 后续迭代清单

- 小红书 Agentic AI 审核系统架构图。
- 小红书证据链 / 模型网关 / 评估平台模块边界图。
- 阿里 ODS 风控数据服务架构图。
- Web3 Risk AI 三个作品集 README。
- 最终一页版简历和 PDF 导出版。
